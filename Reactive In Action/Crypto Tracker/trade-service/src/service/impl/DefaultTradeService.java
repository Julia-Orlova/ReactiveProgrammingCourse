...
public class DefaultTradeService implements TradeService {
	...
	Mono<Void> resilientlyStoreByBatchesToAllRepositories(
		...
		Sinks.Many<Long> delay = Sinks.unsafe().many().multicast()
				.onBackpressureBuffer(1, false);
		Sinks.Many<Long> interval = Sinks.unsafe().many().multicast()
				.onBackpressureBuffer(1, false);
		delay.emitNext(0L, FAIL_FAST);
		interval.emitNext(0L, FAIL_FAST);

		return input.bufferWhen(
				Flux.interval(Duration.ZERO, Duration.ofSeconds(1))
						.onBackpressureDrop()
						.concatMap(s -> Mono.just(s).delayUntil(__ -> interval.asFlux().next()), 1),
				e -> delay.asFlux().zipWith(Mono.delay(Duration.ofMillis(1000)))
			).doOnNext(__ -> logger.warn(".buffer(Duration.ofMillis(100)) onNext(" + __ + ")"))
				.concatMap(trades -> {
					if (trades.isEmpty()) {
						return Mono.empty().doFirst(() -> interval.emitNext(0L, FAIL_FAST))
								.then(Mono.fromRunnable(() -> delay.emitNext(0L, FAIL_FAST)));
					}
					return Mono.zip(
							saveIntoMongoDatabase(tradeRepository1, trades),
							saveIntoRelationalDatabase(tradeRepository2, trades)
					).doFirst(() -> interval.emitNext(0L, FAIL_FAST))
							.then(Mono.fromRunnable(() -> delay.emitNext(0L, FAIL_FAST)));
				}).then();
	}

	Mono<Integer> saveIntoMongoDatabase(TradeRepository tradeRepository1, List<Trade> trades) {
		return tradeRepository1.saveAll(trades).timeout(Duration.ofSeconds(1))
				.retryWhen(Retry.backoff(100, Duration.ofMillis(100))
						.maxBackoff(Duration.ofSeconds(5))
						.filter(exeption -> {
							if (exeption instanceof MongoException) {
								return ((MongoException) exeption).getCode() != 11000;
							} return true;
	...
	Mono<Integer> saveIntoRelationalDatabase(TradeRepository tradeRepository2, List<Trade> trades) {
		return tradeRepository2.saveAll(trades).timeout(Duration.ofSeconds(1))
				.retryWhen(Retry.backoff(100, Duration.ofMillis(100))
